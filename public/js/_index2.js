...
function init() {
  ...
  ball = new THREE.Mesh(new THREE.SphereGeometry(2), new THREE.MeshNormalMaterial);
  scene.add(ball);
...
function getPos(t) {
  var pppos = tube.parameters.path.getPointAt(t);
  var lllookAt = tube.parameters.path.getPointAt((t + 30 / tube.parameters.path.getLength()) % 1).multiplyScalar(scale);
}

function getLookAt(t) {
...
function rollerCoaster() {
  ...
  var time = Date.now();
  //time += (Math.round(Math.sin(time) * 100) + 100);
  ...
  var test = pos.clone();
  ball.position.copy(test.sub(binormal.clone() /*.multiplyScalar(offset)*/ ));
